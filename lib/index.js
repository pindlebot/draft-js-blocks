...
var indentBackward = function indentBackward(editorState, text) {
  var tabSize = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 2;
  var newEditorState = editorState;
  var leftWhitespace = text.match(_leftIndent.LEFT_INDENT_RE)[1];

  var newContentState = _draftJs.Modifier.replaceText(newEditorState.getCurrentContent(), newEditorState.getSelection().merge({
    anchorOffset: 0
  }), "".concat((0, _leftIndent.pad)(leftWhitespace.length + (0, _leftIndent.getDelta)(leftWhitespace.length, 'backward', 2))));
...
var setSelectionToBlock = function setSelectionToBlock(editorState, newActiveBlock) {
  // TODO verify that always a key-0-0 exists
  var offsetKey = _DraftOffsetKey.default.encode(newActiveBlock.getKey(), 0, 0);

  var node = document.querySelectorAll("[data-offset-key=\"".concat(offsetKey, "\"]"))[0]; // set the native selection to the node so the caret is not in the text and
...
exports.setSelectionToBlock = setSelectionToBlock;

var getBlockMapKeys = function getBlockMapKeys(contentState, startKey, endKey) {
...
exports.getBlockMapKeys = getBlockMapKeys;

var getSelectedBlocksMapKeys = function getSelectedBlocksMapKeys(editorState) {
  var selectionState = editorState.getSelection();
  var contentState = editorState.getCurrentContent();
  return getBlockMapKeys(contentState, selectionState.getStartKey(), selectionState.getEndKey());
};

exports.getSelectedBlocksMapKeys = getSelectedBlocksMapKeys;

var blockInSelection = function blockInSelection(editorState, blockKey) {
  var selectedBlocksKeys = getSelectedBlocksMapKeys(editorState);
  return selectedBlocksKeys.includes(blockKey);
};

exports.blockInSelection = blockInSelection;

var insertBlocks = function insertBlocks(editorState, blocks) {
  var block = getCurrentBlock(editorState);
  var content = editorState.getCurrentContent();
  var blockMap = content.getBlockMap();
  var blocksBefore = blockMap.toSeq().takeUntil(function (v) {
  ...
  var newBlockMap = blocksBefore.concat([[block.getKey(), block]], blocks.map(function (block) {
    return [block.getKey(), block];
  }), blocksAfter).toOrderedMap();
  var lastBlockKey = blocks[blocks.length - 1].getKey();
  var selection = editorState.getSelection();
  var newContent = content.merge({
    blockMap: newBlockMap,
    selectionBefore: selection,
